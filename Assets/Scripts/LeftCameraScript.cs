...
public class LeftCameraScript : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		float posX = Mathf.SmoothDamp (transform.position.x, leftCharacter.transform.position.x, ref velocity.x, smoothTimeX);
		float posY = Mathf.SmoothDamp (transform.position.y, leftCharacter.transform.position.y, ref velocity.y, smoothTimeY);
